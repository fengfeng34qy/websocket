(function (undefined) {
  ...
  if (typeof window === 'undefined' || typeof require === 'function') {
    ...
    CollectionRegister = require('collectionRegister');
    exports = Backbone;
    ...
    CollectionRegister = this.CollectionRegister;
    exports = this;
  }
...
Backbone.Collection.prototype.ioBind = function (eventName, io, callback, context) {
  var ioEvents = this._ioEvents || (this._ioEvents = {})
    , globalName = _.result(this, 'name') + ':' + eventName
    , self = this;
  if ('function' == typeof io) {
    context = callback;
    callback = io;
  ...
  CollectionRegister.register(event.global, event.cbGlobal);
  if (!ioEvents[event.name]) {
    ioEvents[event.name] = [event];
  } else {
    ioEvents[event.name].push(event);
